...
module.exports = {
    ...
    run: async (bot, message, args) => {
        ...
        message.channel.bulkDelete(args[0])
            .then(messages => message.channel.send(`**Succesfully deleted \`${messages.size}/${args[0]}\` messages**`).then(msg => msg.delete({ timeout: 1 }))).catch(() => null)
      .then(messages => message.channel.send(`**Unable to use this command, please contact the developer of this bot: !Dark!#4461 and try to explain the problem to him! As it may be a bug/error.**`))
